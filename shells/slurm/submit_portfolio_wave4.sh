#!/usr/bin/env bash
# ====================================================================
# Wave 4 — Portfolio: small sigma (0.01, 0.05) with n_samples + pred_loss_reg
# ====================================================================
# Grid (portfolio only):
#   n_samples         ∈ {1, 5}
#   sigma             ∈ {0.01, 0.05}  (constant — small sigma regime)
#   lr                ∈ {1e-3, 5e-3}
#   pred_loss_weight  ∈ {0, 0.1, 1.0}
#
# 2 n × 2 σ × 2 lr × 3 λ = 24 experiments
#
# Usage:
#   bash shells/slurm/submit_portfolio_wave4.sh --dry-run
#   bash shells/slurm/submit_portfolio_wave4.sh
#   bash shells/slurm/submit_portfolio_wave4.sh --no-backup
# ====================================================================
...
for n_samples in "${N_SAMPLES_LIST[@]}"; do
    for sigma in "${SIGMAS[@]}"; do
        ...
        for plw in "${PRED_LOSS_WEIGHTS[@]}"; do
            ptag="${PLW_TAG[$plw]}"
            yaml="${CFGDIR}/perturb_w4_n${n_samples}_${stag}_${ptag}.yaml"
            ...
            for lr in "${LRS[@]}"; do
                lr_tag="${LR_TAG[$lr]}"
                prefix="sd1_w4_n${n_samples}_${stag}_${ptag}_${lr_tag}"
...
# ---- Main ----
echo "=============================================="
echo "Wave 4 — Portfolio: small sigma × n_samples × pred_loss_reg"
...
if $DRY_RUN; then
    ...
    echo "Monitor with: squeue -u \$USER | grep pco_portfolio_sd1_w4"
fi
