#!/usr/bin/env bash
# ====================================================================
# Wave 2 — Knapsack: n_samples + larger sigma + pred_loss regularisation
# ====================================================================
# Grid:
#   n_samples         ∈ {1, 5}
#   sigma             ∈ {0.5, 1.0, 2.0}  (constant — push the ↑σ trend)
#   lr                ∈ {1e-3, 5e-3}
#   pred_loss_weight  ∈ {0, 0.1}
#
# 2 n × 3 σ × 2 lr × 2 λ = 24 experiments
#
# n_epochs=500, patience=80  (W1 best run exhausted patience=40 at
# epoch 256 while still improving)
#
# Usage:
#   bash shells/slurm/submit_knapsack_wave2.sh --dry-run
#   bash shells/slurm/submit_knapsack_wave2.sh
#   bash shells/slurm/submit_knapsack_wave2.sh --no-backup
# ====================================================================
...
for n_samples in "${N_SAMPLES_LIST[@]}"; do
    for sigma in "${SIGMAS[@]}"; do
        ...
        for plw in "${PRED_LOSS_WEIGHTS[@]}"; do
            ptag="${PLW_TAG[$plw]}"
            yaml="${CFGDIR}/perturb_kn_n${n_samples}_${stag}_${ptag}.yaml"
            ...
            for lr in "${LRS[@]}"; do
                lr_tag="${LR_TAG[$lr]}"
                prefix="sd1_kn_n${n_samples}_${stag}_${ptag}_${lr_tag}"
...
# ---- Main ----
echo "=============================================="
echo "Wave 2 — Knapsack: n_samples × sigma × pred_loss_reg"
