...
SIGMA_COLORS = plt.cm.plasma(np.linspace(0.1, 0.9, len(DEFAULT_SIGMAS)))


# ---- Model builder ----

def build_pred_model(problem):
    """Build a fresh dense pred model matching the problem's model shape."""
    sys.path.insert(0, os.path.abspath("."))
    from openpto.method.Predicts.dense_model import DensePredModel
    in_dim, out_dim = problem.get_model_shape()
    return DensePredModel(in_dim, out_dim)
...
def compute_regret(pred_model, problem, ptoSolver, X, Y, Y_aux, opt_obj, device):
    ...
    for i in range(n):
        ...
        true_obj = float(problem.get_objective(
            Y[i:i+1], np.array(sol)[np.newaxis], Y_aux[i:i+1] if Y_aux is not None else None
        ).mean())
...
def compute_perturbed_obj(pred_model, problem, ptoSolver, X, Y, Y_aux,
    ...
    """
    Estimate the perturbed objective value (expected obj under Gaussian noise on coeff).
    Uses the same forward pass as the perturbed loss: sample z_n from perturbed coeffs,
    compute true objective, return mean normalised regret.
    """
    pred_model.eval()
    X_t = torch.FloatTensor(X).to(device)
    coeff_hat = pred_model(X_t).detach().cpu().numpy()  # (N, D)
    ...
    for i in range(n):
        ...
        for _ in range(n_samples):
            ...
            sol, obj, _ = ptoSolver.solve(c + noise)
            true_obj = float(problem.get_objective(
...
def interpolate_state_dicts(sd_a, sd_b, alpha):
    """Return state dict = (1-alpha)*sd_a + alpha*sd_b."""
    sd = {}
    for k in sd_a:
        sd[k] = (1.0 - alpha) * sd_a[k].float() + alpha * sd_b[k].float()
    return sd
...
def main():
    ...
    parser.add_argument("--n_samples_perturb", type=int, default=20,
                        help="Number of perturbation samples per instance for perturbed obj")
    ...
    for path, name in [(args.perturb_ckpt, "perturbed"), (args.mse_ckpt, "MSE")]:
        if not os.path.exists(path):
            ...
            print("Run submit_kn_landscape_ckpt.sh first to generate it.")
            sys.exit(1)

    # ---- Load problem + solver ----
    sys.path.insert(0, os.path.abspath("."))
    from openpto.method.wrapper_solver import solver_wrapper
    from openpto.problems.Knapsack import Knapsack

    print("Loading problem...")
    problem = Knapsack(
        num_train_instances=320,
        num_val_instances=80,
        num_test_instances=200,
        prob_version="gen",
        loadnew=False,
    )
    solver_kwargs = problem.init_API()
    ptoSolver = solver_wrapper("heuristic", **solver_kwargs)
    ...
    # ---- Build model ----
    pred_model = build_pred_model(problem).to(device)
    ...
    for idx, alpha in enumerate(alphas):
        ...
        sd_interp = interpolate_state_dicts(sd_mse, sd_perturb, alpha)
        pred_model.load_state_dict(sd_interp)
    ...
    for sigma, color in zip(args.sigmas, sigma_colors):
        ax2.plot(alphas, perturbed_objs[sigma], color=color, lw=1.8,
                 label=f"σ={sigma}")
