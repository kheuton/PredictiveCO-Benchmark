"""
Figure: OCV_Y target sweep — 3 learning rates compared.

3-panel figure, one row per model (dense / poly):
...
Usage:
    python rethink_exp/fig_ocv_lr_comparison.py
"""
...
LR_CONFIGS = [
    ('ocv_run1',   '5e-2', '#e66101'),
    ('ocv_lr1e-2', '1e-2', '#5e3c99'),
    ('ocv_lr5e-3', '5e-3', '#1a9641'),
]

STUCK_THRESH = 0.40
MODELS = ['dense', 'poly']
MODEL_COLORS = {'dense': '#2166ac', 'poly': '#d6604d'}


# ---- Load ----

def load_prop(prefix, model):
    rows = []
    pat = os.path.join(BASE_DIR, prefix, f'prop_*_{model}.npz')
...
# ---- Figure: 2 rows (dense / poly) × 3 panels ----
fig, axes = plt.subplots(2, 3, figsize=(18, 10))
fig.suptitle(
    f'OCV_Y adaptive sigma — LR comparison (cubic, proportional mode)',
...
for row_idx, model in enumerate(MODELS):
    ...
    # Shared axis formatting — panel 1
    ...
    # Shared axis formatting — panel 2
...
plt.tight_layout()
out = os.path.join(BASE_DIR, 'fig_ocv_lr_comparison.png')
